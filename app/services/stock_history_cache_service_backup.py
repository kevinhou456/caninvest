...
class StockHistoryCacheService:
    ...
    职责：
    1. 统一管理股票历史价格数据的缓存逻辑
    2. 智能识别缺失数据并从Yahoo Finance补充
    3. 提供高效的历史数据查询接口
    4. 避免重复API调用，减少对外部服务的压力
    ...
    def __init__(self):
        ...
        # 引用全局注册表，确保跨实例共享
        self._recent_fetch_registry = _GLOBAL_FETCH_REGISTRY
        self._recent_failure_registry = _GLOBAL_FAILURE_REGISTRY
        self._no_data_registry = _GLOBAL_NO_DATA_REGISTRY
    ...
    def _record_fetch(self, symbol: str, currency: str, start_date: date, end_date: date, success: bool):
        ...
        old_start = record.get('start')
        old_end = record.get('end')
        ...
        print(f"[记录抓取] {symbol}({currency}) {start_date}->{end_date} {status}")
        print(f"  更新范围: {old_start}->{old_end} => {record['start']}->{record['end']}")
        ...
        for start_date, end_date in merged_ranges:
            try:
                ...
                if self._should_skip_fetch(symbol, currency, fetch_start, fetch_end, force_refresh):
                    print(f"[跳过抓取] {symbol}({currency}) {fetch_start}->{fetch_end} - 冷却期内")
                    continue

                # 根据IPO日期调整查询范围，避免无意义的pre-IPO查询
                adjusted_start, adjusted_end, should_skip = self._adjust_date_range_for_ipo(
                    symbol, fetch_start, fetch_end, currency
                )

                # 如果应该跳过这次查询，直接continue
                if should_skip:
                    continue

                # 检查调整后的日期范围是否已经有数据，避免重复查询

                print(f"[Yahoo Fetch] {symbol}({currency}) {adjusted_start} -> {adjusted_end} (原始缺口 {start_date}->{end_date})")
                if adjusted_start != fetch_start or adjusted_end != fetch_end:
                    print(f"[IPO调整] {symbol} 查询范围已根据IPO日期调整")

                # 从Yahoo Finance获取数据
                raw_data, response_info = self.stock_service.get_stock_history(symbol, adjusted_start, adjusted_end)
                self._register_response_metadata(symbol, currency, response_info, adjusted_start, adjusted_end)
                ...
                if raw_data:
                    ...
                    if success:
                        self._record_fetch(symbol, currency, adjusted_start, adjusted_end, success=True)
                    else:
                        self._record_fetch(symbol, currency, adjusted_start, adjusted_end, success=False)
                else:
                    # 没有获取到数据
                ...
                # 即使发生异常，也要记录这次抓取尝试，避免重复调用
                logger.error(f"抓取 {symbol} 数据时发生异常: {str(e)}")
                self._record_fetch(symbol, currency, adjusted_start, adjusted_end, success=False)
                continue
    ...
    def _adjust_date_range_for_ipo(self, symbol: str, start_date: date, end_date: date, currency: str) -> Tuple[date, date, bool]:
        """
        根据股票IPO日期调整查询的日期范围，避免无效的pre-IPO查询
        ...
        返回:
            Tuple[date, date, bool]: (调整后的开始日期, 调整后的结束日期, 是否跳过查询)
            如果第三个值为True，表示应该跳过这次查询
        """
        try:
            # 查找股票缓存记录中的IPO日期
            stock_cache = StocksCache.query.filter_by(
                symbol=symbol.upper(),
                currency=currency.upper()
            ).first()

            if not stock_cache or not stock_cache.first_trade_date:
                # 如果没有IPO日期信息，尝试网络查询
                if stock_cache and not stock_cache.first_trade_date:
                    logger.info(f"尝试网络查询 {symbol} 的IPO日期")
                    ipo_date = self._query_ipo_date_online(symbol)
                    if ipo_date:
                        stock_cache.first_trade_date = ipo_date
                        from app import db
                        db.session.commit()
                        logger.info(f"已通过网络查询设置 {symbol} IPO日期: {ipo_date}")
                        # 继续使用新获取的IPO日期进行调整
                    else:
                        # 网络查询失败，返回原始日期范围
            ...
            ipo_date = stock_cache.first_trade_date
            today = date.today()

            # 如果结束日期早于IPO日期，说明整个查询范围都是pre-IPO的，完全跳过
            if end_date < ipo_date:
                logger.info(f"跳过 {symbol} 查询：整个范围 {start_date} -> {end_date} 都在IPO日期 {ipo_date} 之前")
                return start_date, end_date, True

            # 如果开始日期早于IPO日期，调整为IPO日期
            if start_date < ipo_date:
                logger.info(f"调整 {symbol} 查询起始日期：{start_date} -> {ipo_date} (IPO日期)")
                start_date = ipo_date

            # 检查最终的日期范围是否太小（少于7天），如果是则跳过
            days_span = (end_date - start_date).days
            if days_span < 7:
                logger.info(f"跳过 {symbol} 查询：日期范围太小 ({days_span}天) {start_date} -> {end_date}")
                return start_date, end_date, True
            ...
            logger.error(f"调整IPO日期范围失败 {symbol}: {str(e)}")
            return start_date, end_date, False




    def _query_ipo_date_online(self, symbol: str) -> Optional[date]:
        """
        通过网络查询股票的精确IPO日期
        ...
        try:
            ...
            # 方法1: 尝试从Yahoo Finance获取IPO信息
            ipo_date = self._query_yahoo_ipo_date(clean_symbol)
            if ipo_date:
                logger.info(f"从Yahoo Finance查询到 {symbol} IPO日期: {ipo_date}")
                return ipo_date

            # 方法2: 尝试从其他金融数据源查询
            # 这里可以添加更多数据源

            logger.warning(f"无法查询到 {symbol} 的IPO日期")
            return None

        except Exception as e:
            logger.error(f"查询 {symbol} IPO日期失败: {str(e)}")
            return None

    def _query_yahoo_ipo_date(self, symbol: str) -> Optional[date]:
        """
        从Yahoo Finance查询IPO日期
        ...
        try:
            ...
            patterns = [
                ...
                # 其他常见格式
                r'IPO\s*[:\s]*([A-Za-z]+\s+\d+,?\s+\d{4})',
                r'IPO\s*[:\s]*(\d{4})',
                r'Public\s+[:\s]*([A-Za-z]+\s+\d+,?\s+\d{4})',
            ]
                    ...
                    try:
                        ...
                        # 处理完整日期格式
                            ...
                            for fmt in date_formats:
                                try:
                                    from datetime import datetime
                                    parsed_date = datetime.strptime(match.strip(), fmt).date()
                                    # 验证日期是否合理
            ...
            logger.debug(f"Yahoo Finance IPO查询失败 {symbol}: {str(e)}")
            return None
